...
root_dir = "/root/siton-tmp/xw_jpg"
...
train_loader = DataLoader(train_dataset, batch_size=1024, shuffle=True, num_workers=8,pin_memory=True, worker_init_fn=_init_fn)
test_loader = DataLoader(test_dataset, batch_size=512, shuffle=False,num_workers=8,pin_memory=True, worker_init_fn=_init_fn)
...
def train_loss_acc(epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        nn.utils.clip_grad_norm_(my_net.parameters(), max_norm=0.9)
        losses += loss.item()
...
best_loss=float('inf')
best_acc=float(0)
path_model = "xwmodel.pkl"
path_state_dict = "xwmodel_state_dict.pkl"

for epoch in range(0, 100):
    my_net.train()
    train_loss, train_acc = train_loss_acc(epoch)
    ...
    with torch.no_grad():
        ...
        if test_loss <= best_loss and test_acc >= best_acc:
            best_loss = test_loss
            best_acc = test_acc
            torch.save(my_net, path_model)
            net_state_dict = my_net.state_dict()
    ...
    scheduler.step()
    print("epoch:{} 训练集准确率：{}，loss：{:.3}, 测试集：{}，loss：{:.3}".format(epoch, train_acc, train_loss, test_acc, test_loss))
